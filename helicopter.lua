--[[pod_format="raw",created="2024-06-09 04:49:34",modified="2024-06-25 21:58:13",revision=5836]]
...
local r22 = apply_materials(unpod(fetch("mdl/r22.pcm")))
local r22_rotor = apply_materials(unpod(fetch("mdl/r22Rotor.pcm")))
local r22_tail_rotor = apply_materials(unpod(fetch("mdl/r22TailRotor.pcm")))
...
local function update()
	...
	local floor_height = get_height(body.position.x,body.position.z)

	if body.position.y <= floor_height then
		if body.velocity:magnitude() > 3 then
			body.position.y = -body.position.y+floor_height*2
			body.velocity.y = abs(body.velocity.y)
			note(68+rnd(3),8,vol,0,0,8,true)
		else
			body.position.y = floor_height
